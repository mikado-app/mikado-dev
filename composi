...
async function loadLineup(gameId, away, home) {
  ...
  try {
    const ar = await fetchJSON(NHL_API + '/roster/' + away.abbrev + '/20252026');
    const hr = await fetchJSON(NHL_API + '/roster/' + home.abbrev + '/20252026');
    const af2 = (ar.forwards||[]).slice(0,12), ad2 = (ar.defensemen||[]).slice(0,6), ag2 = (ar.goalies||[]).slice(0,1);
    const hf2 = (hr.forwards||[]).slice(0,12), hd2 = (hr.defensemen||[]).slice(0,6), hg2 = (hr.goalies||[]).slice(0,1);
    ...
    if (linesDiv) linesDiv.innerHTML = '<div class="compo-pending">Non disponible</div>';
